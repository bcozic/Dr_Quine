...
section .data
	s:
		.s db "%cdefine SYSCALL(nb)		0x2000000 | nb%c%cdefine OPEN			5%c%cdefine CLOSE			6%c%cdefine O_CREAT			0x0200%c%cdefine O_RDWR			0x0002%c%cdefine S_IWUSR			0x0080%c%cdefine S_IRUSR			0x0100%c%csection .data%c	s:%c		.s db %c%s%c, 0%c		.asm_file_raw db %cSully_%cd.s%c,0%c		.compilation1_raw db %cnasm -f macho64 -D NOTFIRST Sully_%cd.s -o obj/Sully_%cd.o%c, 0%c		.compilation2_raw db %cgcc -lSystem -o Sully_%cd obj/Sully_%cd.o%c, 0%c		.execution_raw db %c./Sully_%cd%c, 0%c%csection .bss%c	i:%c		.asm_file resb 32%c		.compilation1 resb 128%c		.compilation2 resb 128%c		.execution resb 32%c%csection .text%c	global start%c	global _main%c	extern _dprintf%c	extern _sprintf%c	extern _system%c%cstart:%c	call _main%c	ret%c%c_main:%c	push rbp%c	mov rbp, rsp%c	push rbx%c	sub rsp, 0x558%c	mov rbx, %d%c%cifndef NOTFIRST%c	inc rbx%c%cendif%c	cmp rbx, 0x7FFFFFFF%c	jg return_sully%c	test rbx, rbx%c	jle return_sully%c	dec rbx%c	lea rdi, [rel i.asm_file]%c	lea rsi, [rel s.asm_file_raw]%c	mov rdx, rbx%c	call _sprintf%c	lea rdi, [rel i.compilation1]%c	lea rsi, [rel s.compilation1_raw]%c	mov rdx, rbx%c	mov rcx, rbx%c	call _sprintf%c	lea rdi, [rel i.compilation2]%c	lea rsi, [rel s.compilation2_raw]%c	mov rdx, rbx%c	mov rcx, rbx%c	call _sprintf%c	lea rdi, [rel i.execution]%c	lea rsi, [rel s.execution_raw]%c	mov rdx, rbx%c	call _sprintf%c	lea rdi, [rel i.asm_file]%c	mov rsi, O_CREAT | O_RDWR%c	mov rdx, S_IWUSR | S_IRUSR%c	mov rax, SYSCALL(OPEN)%c	syscall%c	jc end_sully%c	mov QWORD [rbp-8], rax%c	mov rdi, rax%c	lea rsi, [rel s.s]%c	mov rdx, 37%c	mov rcx, 10%c	mov r8, 37%c	mov r9, 10%c	xor rax, rax%c	mov r10, 170%cret_line:%c	mov QWORD [rsp+rax], 10%c	dec r10%c	add rax, 8%c	test r10, r10%c	jne ret_line%c	mov QWORD [rsp], 37%c	mov QWORD [rsp+0x10], 37%c	mov QWORD [rsp+0x20], 37%c	mov QWORD [rsp+0x30], 37%c	mov QWORD [rsp+0x40], 37%c	mov QWORD [rsp+0x68], 34%c	lea r11, [rel s.s]%c	mov QWORD [rsp+0x70], r11%c	mov QWORD [rsp+0x78], 34%c	mov QWORD [rsp+0x88], 34%c	mov QWORD [rsp+0x90], 37%c	mov QWORD [rsp+0x98], 34%c	mov QWORD [rsp+0xa8], 34%c	mov QWORD [rsp+0xb0], 37%c	mov QWORD [rsp+0xb8], 37%c	mov QWORD [rsp+0xc0], 34%c	mov QWORD [rsp+0xd0], 34%c	mov QWORD [rsp+0xd8], 37%c	mov QWORD [rsp+0xe0], 37%c	mov QWORD [rsp+0xe8], 34%c	mov QWORD [rsp+0xf8], 34%c	mov QWORD [rsp+0x100], 37%c	mov QWORD [rsp+0x108], 34%c	mov QWORD [rsp+0x1d8], rbx%c	mov QWORD [rsp+0x1e8], 37%c	mov QWORD [rsp+0x200], 37%c	call _dprintf%c	lea rdi, [rel i.compilation1]%c	call _system%c	lea rdi, [rel i.compilation2]%c	call _system%c	lea rdi, [rel i.execution]%c	call _system%c	mov rdi, QWORD [rbp-8]%c	mov rax, SYSCALL(CLOSE)%c	syscall%c	jc end_sully%creturn_sully:%c	xor rax, rax%cend_sully:%c	add rsp, 0x558%c	pop rbx%c	leave%c	ret%c", 0
		.asm_file_raw db "Sully_%d.s",0
		.compilation1_raw db "nasm -f macho64 -D NOTFIRST Sully_%d.s -o obj/Sully_%d.o", 0
